...

{
    nixpkgs.overlays = [
    (final: prev: {
      skyrim-cursor-theme = pkgs.stdenv.mkDerivation rec {
        pname = "Skyrim-cursors";
        version = "1.0";
        dontBuild = true;
  ...
  ln -sf ns-resize down-arrow
  ln -sf ew-resize e-resize
  ln -sf ew-resize left-arrow
  ln -sf crosshair left_tee
  ln -sf crosshair ll_angle
  ln -sf crosshair lr_angle
  ...
  ln -sf crosshair top-tee
  ln -sf crosshair ul-angle
  ln -sf ns-resize up-arrow
  ln -sf crosshair ur-angle
  ln -sf text vertical-text
  ln -sf ew-resize w-resize
  ln -sf not-allowed x-cursor
